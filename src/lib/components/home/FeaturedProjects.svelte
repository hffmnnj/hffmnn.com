<script lang="ts">
	...
	import { ArrowRight01Icon, GithubIcon, Globe02Icon } from "@hugeicons/core-free-icons";
	...
	function handleWebsiteClick(e: MouseEvent, url: string) {
		e.preventDefault();
		e.stopPropagation();
		window.open(url, '_blank', 'noopener,noreferrer');
	}
	...
	<div class="max-w-6xl mx-auto">
		...
		<div class="grid md:grid-cols-2 gap-6 items-stretch">
			{#each projects as project, i}
				<a href="/tools/{project.slug}" class="flex flex-col glass-card p-8 group hover:border-white/15 transition-all duration-500 animate-fade-up relative h-full {i === 0 ? 'md:col-span-2 delay-100' : i === 1 ? 'delay-200' : i === 2 ? 'delay-300' : i === 3 ? 'delay-400' : 'delay-500'}">
		...
		<div class="mt-16 animate-fade-up delay-700">
			<div class="flex items-center gap-3 mb-8">
				<div class="w-1 h-8 bg-reins rounded-full"></div>
				<h3 class="font-display text-2xl md:text-3xl font-bold">Reins</h3>
				<span class="text-muted-foreground text-sm">Your AI, your device, your rules</span>
			</div>

			<div class="grid md:grid-cols-2 gap-6">
				{#each reinsProjects as project, i}
					<a href="/tools/{project.slug}" class="flex flex-col glass-card p-6 group hover:border-white/15 transition-all duration-500 animate-fade-up relative h-full {i === 0 ? 'delay-700' : 'delay-800'}">
						<article class="flex-1">
							<div class="flex items-start justify-between mb-4">
								<h4 class="font-display text-lg font-semibold">
									{project.title}
								...
								<div class="flex items-center gap-1.5">
									{#if project.websiteUrl}
										<button
											onclick={(e) => handleWebsiteClick(e, project.websiteUrl!)}
											class="p-1.5 rounded-lg glass-button opacity-60 hover:opacity-100 hover:scale-110 transition-all"
